...
class GeneticAlgorithm:
    ...
    def __init__(self, selection, crossover, mutations, elitism, copy_fract=0, 
        ...
        sd_trigger : Integer >=1
            The number of times in a row the genome similarity should be above the sd_sim
            before triggering a social distaster.
        """
        ...
        self.social_disaster_sim = sd_sim
        self.social_disaster_trigger = sd_trigger
        self.social_disaster_count = 0
    ...
    def evolve_population(self, genomes, fitness, genome_domain):
        ...
        if similarity > self.social_disaster_sim: 
            self.social_disaster_count += 1
            if self.social_disaster_count > self.social_disaster_trigger:
...
if __name__ == '__main__':
    ...
    selection = SelectionTournament(k=5)
    crossover = CrossoverMultipoint(n=2)
    mutations = [
        MutationUniformReplacement(p=0.03), 
        MutationAdjustment(p=0.03, adjustment_range=(-0.1,0.1))]
    genalg = GeneticAlgorithm(
        selection, crossover, mutations, elitism=2, copy_fract=0.1,
        sd_sim=0.8, sd_trigger=8)
