...
class SideBar:
    ...
    def __init__(self, dimensions, background_color, nr_figures):
        ...
        y_pad, x_pad = 10, 10
        fig_height = int((self.height - (1 + nr_figures) * y_pad) / nr_figures)
        fig_width = int(self.width - 2 * x_pad)
        figure_pos = [(x_pad, (i + 1) * y_pad + i * fig_height) for i in range(nr_figures)]
        figure_dim = (fig_width, fig_height)
        self.figures = []
        for pos in figure_pos:
            fig = PlotFigure(self.surface, pos, figure_dim, Color.WHITE)
            fig.clear()
